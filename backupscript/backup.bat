...
SET backuptime=%date:~7,4%-%date:~3,2%-%date:~0,2%-%time:~0,2%-%time:~3,2%-%time:~6,2%
echo %backuptime%
...
`forfiles /p "%backupfldr%" /c "cmd /c if /i @isdir == true echo @path" /d -%retaindays% 2^>nul`
) do echo rd /s /q "%%~G"
