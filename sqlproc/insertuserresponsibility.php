<?php
	...
	$result=mysqli_query($connection, "select count(*) as cnt from misuserresponsibility p 
		where p.misactive=1 and p.dldatetime is null and mod(misresponsibilityid,100)=$users_responsibility and misuserid=$misuserid and misfactoryid=".$_SESSION["factorycode"]);
	$row = mysqli_fetch_assoc($result);
	if ($row['cnt'] > 0) 
	{
		echo 'User already have responsibility';
		exit;
	}
	$connection -> autocommit(FALSE);
	$sql = "insert into swappcoi_db.misuserresponsibility(misuserid,misresponsibilityid,misfactoryid,misactive,cruserid) values ($misuserid,$misresponsibilityid,".$_SESSION["factorycode"].",1,".$_SESSION["usersid"].")";
	if (mysqli_query($connection, $sql)) 
	{
    	$connection -> commit();	
		echo 'User Responsibility is granted <br/>';
	} 
	else 
	{
