...
{	
    ...
    public function __construct(&$connection,$maxlines)
	{
        ...
        $this->pdf->SetSubject('Guarantor List');
        $this->pdf->SetKeywords('GRLST_000.MR');
    ...
    function endreport()
    {
        ...
        $this->pdf->Output('GUARLST_000.pdf', 'I');
    }
	function pageheader()
    {
	...
	function detail()
    {
        ...
        $query = "select * from (select c.contractid,c.contractnumber,
        (select t.transportationvehicleid 
        from contracttransportdetail t
        where t.contractid=c.contractid and t.active=1) as transportationvehicleid 
        from contract c 
        ...
        and exists (select * from contractguarantordetail t 
        where t.active=1 and t.contractid=c.contractid and iscultivator=0
        having count(*)>=0)
        and exists (select * from contractfingerprintdetail t 
        where t.active=1 and t.contractid=c.contractid
        having count(*)>=0))
        order by transportationvehicleid,contractnumber";
		...
		while ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
		{
            if ($contract1->fetch($row['CONTRACTID']))
            {
                ...
                {*/
                    ...
                    $this->pdf->multicell(65,10,$this->vehiclename($row['TRANSPORTATIONVEHICLEID']),0,'L',false,1,170,$this->liney,true,0,false,true,10);
    ...
    function vehiclename($transportationvehicleid)
    {
    	$query = "select n.name_unicode from namedetail n where n.active=1 and n.namedetailid=".$transportationvehicleid;
        $result = oci_parse($this->connection, $query);             
        $r = oci_execute($result);
        if ($row = oci_fetch_array($result,OCI_ASSOC+OCI_RETURN_NULLS))
        {
